...
class EditWindow():
    ...
    def language(self):
        ...
        if languageType == 'CN':
            ...
            self.wrongQuestionMessageVar.set('请输入正确的问题（空白的问题）')
            #
            self.wrongAnswerTitleVar.set('错误的答案')
            self.wrongAnswerMessageVar.set('请输入正确的任答案（空白的答案）')
            # #
            # self.wrongStateTitleVar.set('错误的权重范围')
            # self.wrongStateMessageVar.set('输入正确的权重范围（修改幅度过大）')
            ...
            self.questionExampleVar.set('question(mission id first~)')
            #
            ...
            self.weightExampleVar.set(
                'the weight of the question，as the basis of frequency, it is not recommended to modify the frequency too much,it may cause some problem')
            # error message
            ...
            self.wrongQuestionMessageVar.set('please input the current question（your input is null）')
            #
            self.wrongAnswerTitleVar.set('wrong answer')
            self.wrongAnswerMessageVar.set('please input the current answer（your input is null）')
            # #
            # self.wrongStateTitleVar.set('wrong weight')
            # self.wrongStateMessageVar.set('please input current weight（your input is out of range）')
    ...
    def window(self):
        ...
        def editRecitation():
            ...
            try:
                ...
                if abs(self.weightCheck - weight) > 20:
            ...
            if messagebox.askokcancel(title=self.ifGoOnTitleVar.get(), message=self.ifGoOnMessageVar.get()):
                ...
                self.recitationSystemTools.editRecitation(recitationId=id, question=questionEntry.get(),
                                                          answer=answerEntry.get(), weight=weightEntry.get(),
                                                          isEdit=True)
                # 关闭窗口
